...
if [ "${is_sudo}" -ne 0 ]; then
	sudo_prefix="sudo -S"
	if ! timeout 1 ${sudo_prefix} test > /dev/null 2>&1; then
		sudo_prefix="sudo"
	fi
...
/run/host${host_home}/.local/share/applications/${desktop_home_file}"
	...
	# script.
	host_exec_cmd=$(command -v distrobox-host-exec 2>/dev/null || echo "/usr/bin/distrobox-host-exec")
	"${host_exec_cmd}" --yes update-desktop-database "${host_home}/.local/share/applications" > /dev/null 2>&1 || :
