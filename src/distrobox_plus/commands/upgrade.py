...
def _upgrade_container(
    ...
    Returns:
        Exit code
    """
    # Print upgrade message in red bold like original
    use_color = is_tty()
    if use_color:
        print(f"{RED} Upgrading {container}...{CLEAR}", file=sys.stderr)
    else:
        print(f" Upgrading {container}...", file=sys.stderr)
...
def run(args: list[str] | None = None) -> int:
    ...
    if not container_names and not parsed.all:
        print("Please specify the name of the container.", file=sys.stderr)
        return 1
